...
def stereo_rectify(path):
    ...
    rootpath = path
    keyframe_list = [join(rootpath, kf) for kf in sorted(listdir(rootpath)) if ('keyframe' in kf and 'ignore' not in kf)]
    keyframe_list = [keyframe_list[0]]
    for kf in keyframe_list:
        ...
        for sf in img_filelist:
            ...
            Path(join(rootpath, kf) + '/data/left_finalpass/').mkdir(parents=True, exist_ok=True)
            Path(join(rootpath, kf) + '/data/right_finalpass/').mkdir(parents=True, exist_ok=True)
            Path(join(rootpath, kf) + '/data/reprojection_data/').mkdir(parents=True, exist_ok=True)
