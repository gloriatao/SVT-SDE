...
class Loss(nn.Module):
    def __init__(self, n=1, alpha_AP=0.85, alpha_DS=0.1, alpha_LR=1.0):
        ...
        self.alpha_SPE = 0.1
        self.alpha_SPA = 0.85
        self.mask_l, self.mask_h = self.create_filter(size=(192, 193), r=95)
        self.kernel = self.get_gaussian_kernel(size=21)
    ...
    def fft_L1_loss_mask(self, disp, image):
    ...
    def fft_L1_loss_color(self, image, recon):
        image_gray = image[:, 0] * 0.299 + image[:, 1] * 0.587 + image[:, 2] * 0.114
    ...
    def get_gaussian_kernel(self, size=21):
        ...
        kernel = torch.HalfTensor(kernel).unsqueeze(0).unsqueeze(0)
    ...
    def get_SPE_REC(self, left_pyramid, left_est, right_pyramid, right_est):
        #L1 Loss
        left_l1 = self.fft_L1_loss_color(left_pyramid, left_est)
        right_l1 = self.fft_L1_loss_color(right_pyramid, right_est)
        AP_loss_freq = (left_l1 + right_l1)*0.1
        return AP_loss_freq

    def get_SPE_MAT(self,disp_left, left_pyramid, disp_right, right_pyramid):
        left_l1 = self.fft_L1_loss_mask(disp_left, left_pyramid)
        right_l1 = self.fft_L1_loss_mask(disp_right, right_pyramid)
    ...
    def get_SPA(self, left_pyramid, left_est, right_pyramid, right_est, disp_left, disp_right):
        ...
        left_rec_l1 = self.L1(left_pyramid_high_freq_gray, left_est_high_freq_gray)
        ...
        right_rec_l1 = self.L1(right_pyramid_high_freq_gray, right_est_high_freq_gray)
    ...
    def forward(self, disp, target):
        ...
        SPE_REC_loss = self.get_SPE_REC(left_pyramid, left_est, right_pyramid, right_est)

        # spatial supervision
        SPA_REC_loss = self.get_SPA(left_pyramid, left_est, right_pyramid, right_est, disp_left, disp_right)

        if torch.isnan(LR_loss).sum():
            print('this is a break point!')

        loss_dict = {'AP_loss':AP_loss, 'LR_loss':LR_loss, 'DS_loss':DS_loss,
                     'SPE_REC_loss':SPE_REC_loss,
                     'SPA_REC_loss':SPA_REC_loss, }
        return loss_dict, left_est, right_est
