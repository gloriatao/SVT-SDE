...
def get_model_complexity_info(model, input_res,
    ...
    if input_constructor:
        ...
        # batch = torch.randn([1, 4, 3, 192, 384]).cuda() #!!!!!! unet_vit_input
        # _ = flops_model(batch, batch)


        high = torch.randn([1, 4, 1, 192, 384]).cuda()
        low = torch.randn([1, 4, 3, 192, 384]).cuda()
        x = torch.randn([1, 4, 3, 192, 384]).cuda()
        batch = {'left_high': high, 'right_high': high, 'left_low': low, 'right_low': low,
                 'left_all': x, 'right_all': x}#!!!!!! unet_vit_freq_input
        _ = flops_model(batch)
